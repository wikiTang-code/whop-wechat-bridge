...
// =========================================================================
// 🚀 极速键盘事件 (PageUp / PageDown) 毫秒级历史回溯与换签落盘引擎
// 解决滚轮慢的问题，每秒向上飞翻 10 屏，1 分钟扫完全频道！
// =========================================================================
...
async function run() {
  console.log('========================================================================================');
  console.log('⚡ 启动真实 Chrome 极速键盘事件 (PageUp/Down) 自动翻页落盘');
  ...
  console.log(`\n📋 探测到全部 ${channels.length} 个频道，开始毫秒级极速翻页:`);
  ...
  for (let i = 0; i < channels.length; i++) {
    ...
    const width = 1200;
    const height = 600;
    await whopPage.mouse.click(width / 2, height / 2);

    // 毫秒级极速发送 120 次 PageUp（向上飞速加载更早历史）
    for (let step = 1; step <= 120; step++) {
      ...
      // 发送极速 PageUp
      await whopPage.keyboard.press('PageUp');
      await new Promise(r => setTimeout(r, 200)); // 每秒翻 5 屏！
      ...
      if (step % 30 === 0) {
        ...
        console.log(`  ⚡ [${ch.text}] 极速翻页 [${step}/120] 屏 | 全量已落盘: ${currentOk}/${manifest.length} | 广播就绪: ${broadcastOk}/369`);
      }
    }
  ...
  console.log('\n========================================================================================');
  console.log('📊 全量配图极速翻页换签落盘最终审计看板');
  console.log('========================================================================================');
