...
console.log('====================================================');
console.log('🔥 强力触发：Gemini 5-Key 轮询生成 1.5 万字超深度机构级白皮书');
console.log('====================================================\n');
...
console.log(`🔑 共有 ${keys.length} 个 Gemini Key 可供轮询调度！`);

let playbook = null;
for (let i = 0; i < keys.length; i++) {
  const currentKey = keys[i];
  console.log(`⏳ 正在尝试使用 Key #${i + 1} (${currentKey.substring(0, 8)}...) 请求 Gemini 吐字...`);
  try {
    playbook = await analyzeWithGemini(currentKey, prompt);
    if (playbook && playbook.length > 500) {
      console.log(`🎉 使用 Key #${i + 1} 合成成功！`);
      break;
    }
  } catch (err) {
    console.warn(`⚠️ Key #${i + 1} 遇到了 429 配额限流，将在 2 秒后自动无缝漂移切至下一个 Key...`);
    await new Promise(r => setTimeout(r, 2000));
  }
}

if (!playbook) {
  console.log('⏳ 5 个 Key 均遇到了窗口期，静息等待 22 秒避让打点...');
  await new Promise(r => setTimeout(r, 23000));
  playbook = await analyzeWithGemini(keys[0], prompt);
}

const cleanPlaybook = playbook.replace(/^```markdown\s*/i, '').replace(/^```\s*/i, '').replace(/\s*```$/i, '').trim();

console.log(`🎉 终极生成成功！文本总字数: ${cleanPlaybook.length} 字符`);
