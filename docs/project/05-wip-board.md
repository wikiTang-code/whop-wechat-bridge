...
|:---:|----|:---:|----------|----------|:----:|
| **1** | **CHG-062** | L1 | HOT 释放后不 await 14B；点位指纹去重写 `PENDING_HITL` + 现有 HITL 卡。不改 20/40，不扩正则。GCP 重启不在本刀 | `zhao_print_persist.js` · `hot_defer.js` · `monitor.js` | **done-eng** |
...
| 2 | **REQ-059** | L3 | 轨 2 双账本（S×R；GCP G2 已粘；**负对照**）。`hint_only`；轨 3 未开；`T2R-*` 未改；**`done-strat` 未过** | `059-dual-ledger-track2-report.md` | **done-eng + 负对照** |
...
| 5 | **CHG-053** | L0 | REQ-058 Appendix B 1m resolution diagnostic（docs-only；**不是**主校准表） | `docs/project/058-delayed-follow-e-v0-report.md` | **done-eng** |
| 6 | **CHG-052** | L1 | 前瞻观测 `t_arrive` ingest（新行；禁口播→`px_arrive`；历史仍 `t_arrive_hat`；#16=`e54ffd70`） | `tools/trade/observed_arrival.js` · `database.js` | **done-eng** |
| 7 | **REQ-058** | L3 | Plan B `message_clock` + Appendix A 主校准（5m N=196，门禁 passed）+ Appendix B 1m 诊断（n_scored=33）；**`done-strat` 未过** | `docs/project/058-delayed-follow-e-v0-report.md` | **done-eng (accepted-with-gap)** |
| 8 | **CHG-051** | L0 | 三角色合同。跟单本笔 IREN 46.5=**C**（未追 47.67）。柜台证明另开 `source=counter_smoke` IREN 1 股 **FILLED**（非 zhao_follow）。下一笔赵哥开口须 ask∈40bp 才报送 | `docs/project/dual-track-operating-contract.md` | **accepted** |
| 9 | **CHG-050** | L3 | REQ-057 P2 exploratory holdout。Grok `221a559` ACCEPT WITH NOTES，拍板 B | `scripts/knowledge/lib/exploratory_is_oos.js` | **done-eng (accepted-with-gap)** |
| 10 | **REQ-038-T2** | L3 | n_scored 扩样；03 已由 gemini1 关单 | `card_attribution.js` | **Done** |
| 11 | **REQ-040** | L3 | T2 扩样本；03=`done` | `batch_vision*` · `card_attribution*` | **Done** |
| 12 | **REQ-038-T1** | L3 | Cursor 接管空 SR 重提；03=`done` | `batch_vision*` · `multimodal_context_aligner.js` | **Done** |
| 13 | **DEBT-014** | L3 | HIP 满血暂缓 | `/root/llama.cpp/build-cpu` | Standing |
...
| **Paper FILLED** | 柜台烟测 **过**：`counter_smoke` IREN 1 股字面 FILLED `1286688240727810048` @ 47.075；持仓 IREN=1。跟单本笔仍是 **C**，不是这次成交 | **烟测过 / 跟单 C** |
...
| REQ-003 | L3 | 开盘 GEX 计划任务 | `agent:cursor` | Done | WhopGexOpenSession0940ET Ready；CHG-058 目标 09:35 ET |
...
| CHG-056 | L5 | 长桥 trades-only 1s OHLCV hot jsonl（空秒不写行；`longbridge_trade_agg`） | `agent:cursor` | done-eng | `scripts/market/*` · 部署=Gemini after merge；夹具 #21；非 done-strat |
...
| CHG-059 | L2 | 盘前 GEX 08:45/09:00/09:25 ET | `agent:cursor` | done-eng | win-host-deployed；非 done-strat |
| CHG-060 | L1 | DEBT-026 赵哥点位 persist（`zhao_print`） | `agent:cursor` | done-eng | GCP ingest 待 HITL；#23 改挂本号 |
| CHG-061 | L1 | HOT/WARM/COLD 差速轮询（美股发布+日内波段=WARM 5s） | `agent:cursor` | done-eng | 不改 20/40；背压只打 COLD |
...
| ID | Owner | 日 | 结果 |
|----|-------|-----|------|
| CHG-056b | `agent:cursor` | 2026-09-21 | collect_1s dotenv + subscribe isFirstPush fallback（GCP-verified）；#21 夹具不复称为 live；无非夹具 live bar；非 done-strat |
| CHG-056 | `agent:cursor` | 2026-09-21 | trades-only 1s OHLCV（Grok 钉死：空秒不写；source=longbridge_trade_agg）；非 done-strat；GCP 部署=Gemini after merge；夹具 #21/`969a175` |
| REQ-059 / CHG-055 | `agent:cursor` | 2026-09-21 | 规划合同 + 轨2 脚手架 + GCP G2 负对照摘要已粘（hint_only；轨 3 未开；非 done-strat） |
...
- cursor：**CHG-061** 差速轮询 `done-eng`（HOT 2s / WARM 5s 含美股发布+日内波段；GCP 未部署）。**CHG-060** persist 代码已交。跟单本笔仍 C。Paper IREN=1。
- gemini：033 仍等 Human #91。
- Human：企微 #91；下一笔 ask∈40bp 才 `zhao_follow`。`AUTO_SUBMIT` false。刀 2 未做。#20 GCP G2 摘要 PR 仍开着。
