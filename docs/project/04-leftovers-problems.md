...
| DEBT-025 | **CHG-056 1s hot 未上 GCP**（不挡 Cloud done-eng）：trades-only 采集器与夹具已入库。无 live 成交推送、采集进程不接 rclone、`cold_path=null`。合并后由 **Gemini 部署**。空秒不写行；禁止 1m/5m 插值 1s | `collect_1s_ohlcv.js` · `chg056-market-1s-ohlcv.md` | **P2 部署** |
