...
| DEBT-023 | **REQ-058 假设到达时钟（不挡 done-eng）**：历史仍无观测 `t_arrive`（Appendix A = `t_arrive_hat` / `message_clock`；校准门禁 passed，`done-strat` 未过）。CHG-052 仅给**新** ingest 打观测 `t_arrive`，禁止回填历史。禁止把 C-rate / bar open 当成交秒或 alpha；禁止用 Appendix A 改 20/40 / HUD | `backtest_delayed_follow_e_v0.js` · 058 Appendix A · `observed_arrival.js` | **P3 笔记** |
