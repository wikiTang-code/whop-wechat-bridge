...
| DEBT-026 | **RTH 赵哥点位未进 GCP `trade_signals`（CHG-059 代码已交，云端未 ingest）**：本机 persist 正则+硬锁已落地（`source=zhao_print`；`t_arrive=poll_seen`；`px_arrive` 空；禁 submit/WeCom/`zhao_follow`）。GCP 仍 91 条、最新 9/18，今日 6 笔只在 `messages.poll_seen`。合并+HITL restart 后才有原料。刀 2 企微「我方成交+delta」仅 `zhao_follow`∧FILLED；烟测禁止冒充跟单。不改 20/40 | CHG-059 · `zhao_print_persist.js` | **P1 待 GCP ingest** |
