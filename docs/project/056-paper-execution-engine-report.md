...
| **风控与权限** | 绝对防止程序穿仓、防止错单、防止误报实盘 | `done-eng`：`assertPaperMode` 物理锁死，`AUTO_SUBMIT=false` 门禁，事前 $5k 偏离度拦截 | **未接入账户级动态回撤控制**：当前风控仅限于单笔静态指标（金额/偏离度/标的数），未接入账户当日最大动态亏损熔断机制。 |
| **Alpha 生产** | 摆脱 MFE 幻想，产生可被真实交易验证的策略信号 | `inconclusive`：目前仍依赖大V喊单或启发式雷达 | **策略置信度仍偏低**：根据最新 `readiness-latest.md` 审计，目前战法置信度大多处于 `insufficient`，不能直接用于无人值守全自动发单，必须保持 HITL 介入。 |

### 下一步决策提案 (A/B/C)
- **提案 A (默认推进)**：周一 08:00 CST 准时启动夜盘点火，执行首笔 1 股 TSLA 撮合成交验证，将工程闭环推进为真实交易第一真源闭环。
- **提案 B**：夜盘暂不报单，仅观察长桥夜盘行情推送与盘口刷新稳定性，待常规交易盘 (RTH 21:30 CST) 再进行成交测试。
- **提案 C**：在此之前增加账户级当日动态最大回撤风控拦截（例如当日模拟盘总亏损 > $1,000 时停机）。
