...
| 队列 | 顺位 1 | 状态 |
|------|--------|:----:|
| **§0.A 协同队列** | **REQ-028** 落地 Paper 状态机（TTL/滑点） | 待确认开工（接续） |
| **§0.B `agent:gemini`** | **REQ-003** 开盘 GEX 计划任务（避让 L1 热点） | 待确认开工 |
| **§0.H `human`** | REQ-002 生产 ff | 等待 |
| **§0.R-A / §0.R-B** | 交叉 Review+修复（审对方 Done 包） | 空（待阈值/专题包触发） |

共享候选：`REQ-028`（就绪）· `REQ-029`+`CHG-009` · `REQ-021` · `REQ-008`；已完成近期：`REQ-027` 三账本隔离。  
**交叉审修**：累计出队 ≥5 或专题包关闭 → 移交对方 §0.R（`CHG-012` / 06 §1.2）。
