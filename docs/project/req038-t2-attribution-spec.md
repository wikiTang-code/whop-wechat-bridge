...
| 条件 | 规则 |
|------|------|
| 标的 | **标题 / 触发 / 源消息正文** 出现独立词 **TSLA / TSLL / SOXL / IREN / NBIS / QQQ / SPY / NVDA**（CHG-036 扩样；大小写不敏感）。**不**凭 `tickers_json` 入选 |
| 发送者硬锁（CHG-033） | 源消息 `messages.sender_id` **必须** = `user_4yeplXgbguTu4`（赵哥）；否则 `skipped_non_zhao`。禁模糊名匹配；周哥参考卡不得进 T2 胜率样本 |
| 价位范围 | TSLA/SPY/QQQ/NVDA ∈ [50,900]；TSLL ∈ [1,200]；SOXL/IREN ∈ [5,200]；NBIS ∈ [20,500] |
