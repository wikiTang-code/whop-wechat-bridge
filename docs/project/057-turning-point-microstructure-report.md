...
| SOXL | 0.80, 1.0 | 18 / +2.69 / 5.6% / 6.7% | 7 / +1.87 / 42.9% / 13.6% | **0.08** | 40 |
...
可写入 07 的结论：宽窗 79% 作废；过滤后 5m 30m precision 仍是个位数到四十、unique recall 多 <10%；OOS 固定持有在 IREN/CRWV/COHR 为负；置换未在 α=0.05 拒绝「随机打点也会碰上」。
...
| **柜台真源** | 「回测通过 = Paper 已通」 | 回测与柜台正交。当前时刻仍早于周日 20:00 ET 夜盘。 | 无 FILLED 则无持仓 SoR。 | **P0 不变**：夜盘 `night_market_kickoff.js` 1 股 TSLA。 |
...
2. **P1 Intent 字段**：`t_arrive`, `px_zhao`, `px_arrive`, `delta_px`, `exec_policy`；
3. **仍不必做**：把 38.2% 止盈、OOS PF、量能吸筹写进规则/HUD。
