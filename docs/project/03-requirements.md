...
| CHG-049 | `done` | **吸收 REQ-055 审阅意见与实战盘感，实施持仓推演去神化、来源三层标注、2x名义敞口披露与时变股性徽章落地（P0/P0.5 整改收口）**：落实 Grok × Human 审阅与一线顶级盘感，杜绝纪律过度工程化；彻底废除全局统一死板的固定比例止损与做T接回，改为以盘口异动分批进出为准；API 与 UI 显式标注 source 分层（`audited_fill` / `zhao_quote` / `heuristic`）；重点标的展示收敛至 <=8 活跃焦点，其余历史标的折叠；披露 2x 战车（TSLL/CONL等）名义杠杆暴露（`notionalExposureRatio`）；落地 `tools/knowledge/stock_elasticity_analyzer.js` 动态统计大V近 14/30 天操作与提及频次，挂载光模块/AI云/存储/加密云等时变股性徽章；严格维持与四维共振打分隔离；单测 `test_radar_hud_api.js` 与 `test:local-ops` 100% 绿灯 | 2026-09-20 `agent:gemini` · `tools/trade/position_lifecycle_manager.js` · `monitoring/readonly-api-router.js` · `public/radar_hud.html` · `tools/knowledge/stock_elasticity_analyzer.js` · `test/test_radar_hud_api.js` |
