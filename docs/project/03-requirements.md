...
| CHG-050 | `done-eng` | **REQ-057 P2 方法学补丁（exploratory IS/OOS holdout，禁止称 Walk-Forward/工业级/alpha）**：日历交易日 40/20、ATR/C 分位数 k、下轨用 i-1、IS 预登记网格冻结、OOS 只读、最近邻 TP/FP/FN、时间戳单位断言、置换 B=200、Top5×{5m,1m}、ExitB 报 n/盈亏/MaxDD；057 §7 宽窗表标 `deprecated_wide_window`。单测 `test:chg050-is-oos` 全绿。**accepted-with-gap**：OOS 小样本、置换未过 0.05、1m 仅约 7 会话 | 2026-09-20 `agent:cursor` · `scripts/knowledge/lib/exploratory_is_oos.js` · `scripts/knowledge/backtest_walk_forward_rigorous.js` · `docs/project/057*` · `data/runtime/chg050_*` |
