...
| REQ-056 | P1 | L1/L5 | `accepted-with-gap` | **长桥模拟盘执行闭环**：状态机/风控/对账 `done-eng`。**柜台烟测 FILLED 过**（`counter_smoke` IREN 1 股 @ 47.075，订单 `1286688240727810048`，持仓 1，对账 0）。**跟单门未过**（IREN 46.5=C，无 `zhao_follow`）。`AUTO_SUBMIT=false`。submit→cancel 缺口以此烟测关。禁止把烟测写成已经能跟单 | `brokers/longbridge.js` · `tools/trade/paper_execution_engine.js` · `docs/project/056-paper-execution-engine-report.md` |
...
| CHG-059 | `done-eng` | **DEBT-026 赵哥成交点位落 `trade_signals` + Intent（极窄）**：硬锁 `sender_id=user_4yeplXgbguTu4` + 两大交易频道；正则解析「价格+(加了\|出掉)+ticker」（无 LLM）；`t_arrive=poll_seen`；`px_arrive` 空到着成交；`source=zhao_print`（禁 `zhao_follow` / `counter_smoke`）；forum 写 Intent `PENDING_HITL` 且不 submit；chat 只写 signal；`AUTO_SUBMIT` 仍 false；不改 20/40、不发企微跟单成功。GCP ingest 待 HITL deploy / `--apply` 回填。**`done-strat` 未过**（代码落表 ≠ 追上赵哥） | 2026-09-21 `agent:cursor` · `tools/trade/zhao_print_persist.js` · `monitor.js` · `test/test_zhao_print_persist_chg059.js` |
