...
| REQ-059 | P1 | L3 | `done-eng` | **轨 2 双账本最小集**：S 与 R 物理分列。预注册 4 条冻结 OHLCV（`T2R-*` **未改**）。G2 GCP 摘要已粘（PR #19=`d902722`；`n_messages=109333` `n_events=27` `events_source=sqlite-ro#messages.created_at`；S 30m prec≤~1% / perm p~0.5–1；R OOS PF≤~1；独立期望门未过）。**负对照**（CHG-050 兄弟）。`hint_only`。**轨 3 未开**。**`done-strat` 未过**。禁 HUD / 20/40 / place_order / 绿表改参 / 复现赵哥 / AI 扫单 / 工业级 WF | `docs/project/059-dual-ledger-track2-report.md` §2.3 · 合同 `09-development-plan-contract-20260921.md` |
