...
| CHG-056 | `done-eng` | **长桥 quote/trade 进程内 1s OHLCV hot 采集**：`Period` 最小 `Min_1`，禁止 1s K 线 API / Yahoo 插值 / 十档 / 80 标的 / systemd 宣称存储已 live / 仓内 rclone token。5 标的写入 `data/market/hot/{SYM}/1s/YYYY-MM-DD.jsonl` + `data/manifest/market_1s.jsonl`（`source=longbridge_quote_trade_agg`，`cold_path=null`）。断线重连不回填。Cloud 交脚本+夹具；**GCP 部署=Gemini after merge**。禁 HUD / 20/40 / T2R / place_order / Paper 混车道。**`done-strat` 未过** | 2026-09-21 `agent:cursor` · [`chg056-market-1s-ohlcv.md`](./chg056-market-1s-ohlcv.md) · `scripts/market/collect_1s_ohlcv.js` |
