...
| CHG-019 | `in_progress` | **`saveTradeSignal` ON CONFLICT 补齐 `ticker`/`action`/`source`/`stop_loss`/`speaker_*` 等**（二次纠错改标的不残留旧字段；来自 07 REQ-035 抽审尾巴） | 2026-09-19 `agent:cursor` · `database.js` · `test_trade_signals_req031.js` |
