...
| CHG-018 | `accepted` | **统一 WSL2 AI 运行时架构升级方案**：消灭 Windows LM Studio 7G 物理内存常驻，WSL 内 14B↔1.5B 飞轮时分复用；保留 `:8080`；**§6 门禁已达标**（`7da433a` Adapter+ROCm smoke+SOP）；**待 Step 1–3 + Q-007**（`WslLlamaAdapter` load/unload 仍为 `/tmp` 占位，见 07 抽审） | 2026-09-19 提案 `agent:gemini` · 审阅/抽审 `agent:cursor` · `wsl-unified-ai-runtime-plan.md` |
