...
| CHG-051 | `accepted` | **三角色运行合同**：跟单执行（赵哥到达价 A/B/C）≠ 周哥 QQQ **模拟仓参考频道**（可验证、hint_only、不进 trade_signals）≠ 自研 REFERENCE_ONLY 播报。赵哥单=成交后广播。CHG-050 冻结为负对照。次序：`FILLED` → Intent → delta 账本 → 再谈自研参考轨。**2026-09-21 RTH**：钉 IREN BUY 46.5，长桥 ask 47.67=policy C，未报送、未造 FILLED | 2026-09-20 `agent:cursor` · [`dual-track-operating-contract.md`](./dual-track-operating-contract.md) |
