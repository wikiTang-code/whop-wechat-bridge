...
| REQ-056 | P1 | L1/L5 | `done-eng` | **长桥模拟盘（Paper Trading）执行闭环与 TradeIntent 最小执行状态机工程落地（Phase 0 Week 1）**：落实 Grok 审阅意见，实现不可变意图状态机、长桥原生 SubmitOrderOptions 适配、事前硬风控引擎（$5,000上限/期权拦截/10%滑点截断）、全时段 OutsideRTH（含周日夜盘）、三方对账哨兵与 HUD 驾驶舱双轨看板；19套单测全绿，夜盘一键点火工具就绪 | `brokers/longbridge.js` · `tools/trade/paper_execution_engine.js` · `tools/trade/paper_risk_guard.js` · `tools/ops/paper_reconciliation_sentinel.js` · `scripts/trade/night_market_kickoff.js` · `docs/project/056-paper-execution-engine-report.md` |
...
| REQ-059 | P1 | L3 | `done-eng` | **轨 2 双账本最小集**：风格账 S（规则后 5/15/30min 赵哥开口 + 置换；禁 ±2h）与期望账 R-precursor（次根开、10bp、12 根 vs 回撤分列、日历 40/20、无视开口）物理分列。预注册 4 条冻结 OHLCV 规则（量能进 if）。Cloud=脚本+夹具+报告骨架；全量 archive 在本机/GCP。**`done-strat` 未过**。禁 HUD / 20/40 / place_order / 复现赵哥 / AI 扫单 / 工业级 WF | `scripts/knowledge/lib/dual_ledger_track2.js` · `docs/project/059-dual-ledger-track2-report.md` · 合同 `09-development-plan-contract-20260921.md` |
