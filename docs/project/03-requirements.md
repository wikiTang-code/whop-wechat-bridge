...
| REQ-058 | P1 | L3 | `done-eng` | **延迟跟单历史回测 v0（E-layer）**：`t_arrive_hat=t_msg+Δ`，Δ∈{0,1,3,5}m；`px_arrive`=bar open。Plan B：`--t-msg-kind message_clock` 缺 archive/`messages.created_at` fail-closed；L2a 仅显式反例。**不是**观测 `t_arrive` / 不是 alpha / 不接 HUD·L2a·place_order | `scripts/knowledge/backtest_delayed_follow_e_v0.js` · `docs/project/058-delayed-follow-e-v0-report.md` |
