...
| REQ-058 | P1 | L3 | `done-eng` | **延迟跟单历史回测 v0（E-layer）**：`t_arrive_hat=t_msg+Δ`。Plan B fail-closed。Appendix A：`arrival_kind=hypothesized` + `t_msg_kind=message_clock` 校准表（N=196 BUY / 60d；校准门禁 passed）。**`done-strat` 未过**。禁 HUD / 改 20/40 / 宣称跟单可复制 | `docs/project/058-delayed-follow-e-v0-report.md` · Appendix A |
...
| CHG-052 | `done-eng` | **前瞻观测 `t_arrive` ingest**（REQ-058 跟进）：新 Intent / `trade_signals` 写入墙钟 `t_arrive`（已有 poll-seen 则保留）；`px_arrive` 默认为空，禁口播/K 线回填。历史 E-layer 仍 `t_arrive_hat`，不回填。不改 20/40、不 HUD、不宣称 `done-strat` | 2026-09-21 `agent:cursor` · `tools/trade/observed_arrival.js` · `test/test_observed_t_arrive_chg052.js` |
