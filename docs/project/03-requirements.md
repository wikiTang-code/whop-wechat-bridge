...
| CHG-023 | `in_progress` | **WSL AI 切流锁定**：Windows `127.0.0.1:8080` 独占指向 WSL `llama-server`；默认 `AI_RUNTIME_BACKEND=wsl`；废 8081 bridge；开机 `cutover`→`ssh -R 8080`；方案 §7 拓扑权威 | 2026-09-19 `agent:cursor` · `tools/wsl-ai-cutover.js` · `scripts/whop-lm-tunnel.bat` |
