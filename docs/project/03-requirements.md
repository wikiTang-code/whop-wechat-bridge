...
| REQ-052 | P1 | L3 | `done-eng` | **基于真实交易单（$t_0$）的逆向特征挖掘与高置信战术假说提纯引擎（提案A落地）**：落实用户拍板提案 A，不再盲目从 4000 张散文卡碰运气；聚焦近 60 天有确定时点 $t_0$ 的 388 笔真实交易单，反查当时赵哥原话与社群上下文、时区特征（早盘捡漏/尾盘扫单/盘中异动）、以及高频 K 线微观空间，提纯 2~3 个有真金白银交易支撑的高置信战法假说（形成确凿 Trigger / Entry / Exit / StopLoss 规则）；计算真实回测成效，附带完整 Strategic Gap Audit | `scripts/knowledge/reverse_mine_tactics_from_trades.js` · `docs/project/052-reverse-tactical-mining-report.md` |
| REQ-053 | P1 | L3 | `done-eng` | **实战战术持仓动态生命周期状态机（Position Dynamic Lifecycle Manager）**：将 REQ-052 提纯出的 TAC-001（1/6常规仓试探）、TAC-002（盈利直线拉升半仓阶梯止盈）、TAC-003（同标的高抛低吸差价加回做T）工程化落地为标准状态机；自动维护动态均价、已实现收益、保本损与下一建议动作；单测 100% 绿灯，附带完整 Strategic Gap Audit | `tools/trade/position_lifecycle_manager.js` · `test/test_position_lifecycle_manager.js` · `docs/project/053-position-lifecycle-report.md` |
| REQ-054 | P1 | L3 | `done-eng` | **人工审核增量联动与利润重算流水线（闭环 DEBT-021）**：建立企微人工审核优先覆盖机器初判机制；根据纠偏与跳过权威重算 746 对配对流水；沉淀 90 组高质量 SLM 问答与 DPO 对；52 套单测全绿 | `tools/trade/audit_linked_pnl_pipeline.js` · `test/test_audit_linked_pnl_pipeline.js` |
| REQ-055 | P1 | L3 | `done-eng` | **审核进度每10%里程碑自迭代微调与实战对账单生成机制**：响应用户指示，当人工审核总进度每跨越 10% 梯度（当前 10.84%），自动触发一次 LoRA 增量抽取与微调流水线，版本注册为 `v1.1.0-audit-10pct`，并自动生成 `docs/project/055-slm-audit-milestone-10pct-report.md` 附带完整 Strategic Gap Audit | `scripts/slm/flywheel_engine.js` · `test/test_slm_milestone_pipeline.js` · `docs/project/055-slm-audit-milestone-10pct-report.md` |
| REQ-056 | P1 | L1/L5 | `accepted-with-gap` | **长桥模拟盘执行闭环**：状态机/风控 `done-eng`。**烟测 FILLED 过**（counter_smoke IREN 1 @ 47.075，持仓 1）。**跟单门未过**。`AUTO_SUBMIT=false` | `brokers/longbridge.js` · `paper_execution_engine.js` · `056-paper-execution-engine-report.md` |
| REQ-057 | P1 | L3 | `done-eng` | **赵哥微观转弯战法量化与独立检测器**（`turning_v1` 与执行安全解耦）。**CHG-050**：exploratory IS/OOS holdout 已落地；**禁止**称 Walk-Forward / 工业级 / alpha。宽窗 79% = `deprecated_wide_window`。见 057 报告 | `tools/trade/turning_detector.js` · `scripts/knowledge/lib/exploratory_is_oos.js` · `docs/project/057-turning-point-microstructure-report.md` |
| REQ-058 | P1 | L3 | `done-eng` | **延迟跟单历史回测 v0（E-layer）**：`t_arrive_hat=t_msg+Δ`。Plan B fail-closed。**主校准=Appendix A**（5m N=196；校准门禁 passed）。**Appendix B=1m resolution diagnostic**（`n_scored=33`，不是主表）。**`done-strat` 未过**。禁 HUD / 改 20/40 / 宣称跟单可复制 | `docs/project/058-delayed-follow-e-v0-report.md` · Appendix A/B |
| REQ-059 | P1 | L3 | `done-eng` | **轨 2 双账本最小集**：S 与 R 物理分列。预注册 4 条冻结 OHLCV（`T2R-*` **未改**）。G2 GCP 摘要已粘（`n_messages=109333` `n_events=27`；S 30m prec≤~1% / perm p~0.5–1；R OOS PF≤~1；独立期望门未过）。**负对照**。`hint_only`。**轨 3 未开**。**`done-strat` 未过**。禁 HUD / 20/40 / place_order / 绿表改参 | `docs/project/059-dual-ledger-track2-report.md` §2.3 · 合同 `09-development-plan-contract-20260921.md` |
...
| CHG-046 | `done` | **消化 Grok 审阅意见，实施黄金战法 tier 分级物理门禁与抽检验真闭环（ACCEPT WITH NOTES 验收通过）**：落实外部审阅意见，杜绝粗细战法混淆与虚假共振；`card_attribution.js` 导出结构显式标注 `tier: 'golden_level'` (175张带点位高精战法) 与 `tier: 'golden_direction'` (310张宏观多空方向战法)；四维共振雷达 `tape_confluence_detector.js` 设物理门禁，仅 `golden_level` 享受 D2 顶格加权（25分），`golden_direction` 降档封顶 15 分且防叠加；空间偏差 ≤3% 规范与单测断言写死；带 tier 的 `golden_playbook.json` 重新推产至 GCP VM；抽检验真产出 `sample_audit_report.json`（定性为 Smoke 验收，建立持续抽检协议）；049 保持隔离；单测全绿 | 2026-09-20 `agent:gemini1` · `tools/knowledge/*` · `data/runtime/*` · `test/*` |
| CHG-047 | `done` | **双重交付门禁（Dual-Gate DoD）与战略差距对账规范落地 + 极简就绪度审计脚本 tools/ops/system_readiness_audit.js + 盘中哨兵前瞻落盘闭环**：消化用户与 Grok 批评，严防阶段交付幻觉与信息黑盒；改 `AGENTS.md` 与 `06-process.md` 钉死双重门禁（工程 `done-eng` vs 战略 `done-strat`）；严禁将 MFE 冲高误读为实战胜率；实现只读审计脚本落盘 `readiness-latest.md`；`live_radar_sentinel.js` 挂载 `appendForwardEventWindowBar` 开启日内滚雪球累积切片；单测全绿 | 2026-09-20 `agent:gemini` · `AGENTS.md` · `06-process.md` · `tools/ops/system_readiness_audit.js` · `tools/knowledge/live_radar_sentinel.js` · `readiness-latest.md` |
...
| CHG-051 | `accepted` | **三角色运行合同**。2026-09-21 拍板：跟单本笔 IREN 口播 46.5 vs ask 47.67=**C 弃单**（禁止贴 47.67 冒充跟单）。柜台证明另开 `source=counter_smoke`、`px_zhao=null`，IREN 1 股字面 FILLED（订单 `1286688240727810048` @ 47.075，持仓 1）。FILLED ≠ 追上赵哥。下一笔须 ask∈40bp 才 `zhao_follow` | 2026-09-20 `agent:cursor` · [`dual-track-operating-contract.md`](./dual-track-operating-contract.md) |
| CHG-052 | `done-eng` | **前瞻观测 `t_arrive` ingest**（REQ-058 跟进）：新 Intent / `trade_signals` 写入墙钟 `t_arrive`（已有 poll-seen 则保留）；`px_arrive` 默认为空，禁口播/K 线回填。历史 E-layer 仍 `t_arrive_hat`，不回填。不改 20/40、不 HUD、不宣称 `done-strat` | 2026-09-21 `agent:cursor` · `tools/trade/observed_arrival.js` · `test/test_observed_t_arrive_chg052.js` · merge #16 = `e54ffd70` |
| CHG-053 | `done-eng` | **REQ-058 Appendix B 1m resolution diagnostic（docs-only）**：不是主校准表。主基线仍是 Appendix A / 5m N=196。`n_scored=33`，Yahoo 约 7 个会话；禁止与 5m 的 196 笔逐格当「同一批更精细」。`arrival_kind=hypothesized`；A 仍是 stub。阈值 / HUD / `done-strat` 仍冻。不改代码、不改 20/40、不改 A/B/C policy | 2026-09-21 `agent:cursor` · `docs/project/058-delayed-follow-e-v0-report.md` Appendix B |
| CHG-054 | `done-eng` | **观测 `t_arrive` ingest 落库审计**（#16/#17 跟进；E-docs closed）：live first-see（poll-seen）写入 SQLite；`saveTradeIntent` 持久层禁口播/`bar_open`→`px_arrive`；历史 `manual_correct` 等不发明钟。只读 `scripts/trade/check_observed_t_arrive.js`。不扩 058、不改 20/40/HUD/`done-strat` | 2026-09-21 `agent:cursor` · `database.js` · `observed_arrival.js` · `monitor.js` · `check_observed_t_arrive.js` |
| CHG-055 | `accepted` | **开发规划合同落盘（2026-09-21，HEAD≈e1656af）**：§0–8 背景/缺口/目标与非目标/冻结/方法/轨0–5+DEBT-LLM-L2/阶段门/核验/下一刀=只切轨2。Cloud 交脚本+夹具+报告骨架；全量 `whop_archive.db` 跑在本机/GCP 再粘摘要。不改 20/40、不 HUD、不 Δ 扫、不 place_order | 2026-09-21 `agent:cursor` · [`09-development-plan-contract-20260921.md`](./09-development-plan-contract-20260921.md) |
| CHG-056 | `done-eng` | **长桥成交进程内 1s OHLCV hot 采集（Grok 钉死）**：OHLC **只来自 trades**（Quote 只可心跳，禁止 bid/ask mid 填空秒）；空秒不写行；`source=longbridge_trade_agg`。禁止 1s K 线 / `Period.Second` / 1m·5m 插值 / 十档 / 80 标的 / 采集进程 rclone / systemd 宣称已 live。5 标的 `data/market/hot/{SYM}/1s/YYYY-MM-DD.jsonl`。Cloud 交脚本+夹具；**GCP 部署=Gemini after merge**。夹具验收在 **#21 / `969a175`**，不复称为 live。**`done-strat` 未过** | 2026-09-21 `agent:cursor` · [`chg056-market-1s-ohlcv.md`](./chg056-market-1s-ohlcv.md) · `scripts/market/collect_1s_ohlcv.js` |
| CHG-056b | `done-eng` | **collect_1s live 订阅补丁（GCP dirty diff 核验，非自造 NAPI）**：`dotenv.config()` 从 cwd/repo root 读 `.env`（不入库）；live `subscribe(symbols, types, true)` 第三参 isFirstPush（Longbridge NAPI / GCP live probe），失败回退两参。不改 `ohlcv_1s.js` / `brokers/longbridge.js` / 聚合/空秒/T2R/20/40/HUD/`place_order`。#21 夹具验收不复称为 live。当前 hot IREN/SOXL 为夹具行，尚无非夹具 live bar。**`done-strat` 未过** | 2026-09-21 `agent:cursor` · `scripts/market/collect_1s_ohlcv.js` · 跟进 #21/`969a175` |
| CHG-057 | `done-eng` | **GEX 矩阵扩到赵哥高频正股以作 L2 映证**：默认 matrix=`TSLA,IREN,CRWV,MU,COHR,SOXL,COIN,NVDA,LITE`；2× 填单映正股（TSLL→TSLA、CONL→COIN、NVDL→NVDA）。OpenD 期权链节流 3.5s + 限频重试。摘要/企微/看板列出全部 matrix，不灌 ladder。不拦截、不下单、不替代赵哥点位。**`done-strat` 未过**（映证通道就绪 ≠ 点位共振有效） | 2026-09-21 `agent:cursor` · `collect_futu.py` · `gex-readonly.js` · `adapters/gex.js` |
| CHG-058 | `done-eng` | **开盘 GEX 目标从 09:40 ET 提前到 09:35**：OI 是 T+1，10 分钟等待不刷新库存；2026-09-21 赵哥首簇 09:32–09:39。任务仍名 `WhopGexOpenSession0940ET`，唤醒 09:33、等到 09:35，预告窗 60s。HTML 不入库。对照包 [`chg057-gex-zhao-corroboration-20260921.md`](./chg057-gex-zhao-corroboration-20260921.md) | 2026-09-21 `agent:cursor` · `open_session_run.py` · `install_open_session_task.ps1` |
| CHG-059 | `done-eng` | **盘前 GEX**：唤醒 08:45 ET，09:00 ET 拉链，09:25 ET 截止（晚则 `deadline_status=late`），`spot_session=premarket`。OI=前收 T+1，时效对齐赵哥约 09:32，不是新 OI。09:31 现货刷新禁止重拉 OI（无拆分则 stub）。DST 用 `America/New_York`。仍在 win-host/OpenD。任务名不变，本机重跑安装器才换点。**`done-strat` 未过**（云端未打 OpenD，不能证明 09:25 前落盘） | 2026-09-22 `agent:cursor` · `open_session_run.py` · `install_open_session_task.ps1` · `test/test_open_session_preopen.py` |
| CHG-060 | `done-eng` | **DEBT-026 赵哥成交点位落 `trade_signals`+Intent**（#23 原占 CHG-059，GEX 盘前已占用 059，本号改挂）。硬锁赵哥+两大交易频道；`t_arrive=poll_seen`；`px_arrive` 空；`source=zhao_print`；不 submit、不发企微跟单成功。GCP ingest 待 HITL。**`done-strat` 未过** | 2026-09-23 `agent:cursor` · `zhao_print_persist.js` · `monitor.js` · `test/test_zhao_print_persist_chg059.js` |
| CHG-063 | `done-eng` | **企微模拟盘待确认卡**：`PENDING_HITL` 之后 `deferOffHot` 推到 `WECOM_TRADE_WEBHOOK_URL` 群机器人。文案写模拟盘，按钮只有确认Paper / 放弃。ask 相对 `px_zhao` ≤40bp 才提交 Paper，否则记 C。不走 real EXECUTE，不自动下单，不发跟单成功。**合入前确认链路未通。`done-strat` 未过** | 2026-09-24 `agent:cursor` · `paper_wecom_card.js` |
| CHG-062 | `done-eng` | **HOT 不占 14B**：临界区只做差分拉取、`poll_seen`、正则 `zhao_print`、原文推送，然后释放 `syncing`。`extractAndExecuteTrades` 进 `deferOffHot`。Intent 按 sender+ticker+side+px_zhao+2 分钟桶去重；任一交易频道先到的 `zhao_print` 写 `PENDING_HITL` 并挂现有 HITL 卡。不 `AUTO_SUBMIT`，不发跟单成功，不扩正则。**`done-strat` 未过** | 2026-09-24 `agent:cursor` · `zhao_print_persist.js` · `hot_defer.js` · `monitor.js` |
| CHG-061 | `done-eng` | **分频道差速轮询**：HOT 2s×5条（历史记录区+不用翻墙期权）；WARM 5s×10条（**不用翻墙美股发布**算实时信号 + 日内波段 5s + 股票分析）；COLD 30s/120s×20条。三独立循环挂在 **`ingest_runner`**（线上入口，不是只挂 server.js）。差分跳过、keepAlive、附件延迟、背压只打 COLD。新闻/画像只跟 COLD。不改 20/40、`AUTO_SUBMIT=false`。**`done-strat` 未过** | 2026-09-23 `agent:cursor` · `tier_poller.js` · `ingest_runner.js` · `server.js` |
