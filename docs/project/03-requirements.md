...
| CHG-061 | `done-eng` | **分频道差速轮询**：HOT 2s×5条（历史记录区+不用翻墙期权）；WARM 5s×10条（**不用翻墙美股发布**算实时信号 + 日内波段 5s + 股票分析）；COLD 30s/120s×20条。三独立循环、最新 id 差分跳过、keepAlive、附件延迟 ISR、背压只打 COLD。不改 20/40、`AUTO_SUBMIT=false`。**`done-strat` 未过**（云端未测 429） | 2026-09-23 `agent:cursor` · `channel_registry.json` · `server.js` · `monitor.js` · `channel_poll_config.js` |
