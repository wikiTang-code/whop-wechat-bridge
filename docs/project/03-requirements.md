...
| CHG-060 | `done-eng` | **DEBT-026**：9/21 开盘 6 笔已 `--apply` 进 GCP（12 行 `zhao_print`，`px_arrive` 空，`t_arrive=poll_seen`）。买入 Intent=`PENDING_HITL`，未 submit。今日新口播 0，实时落表未证。**`done-strat` 未过** | 2026-09-24 `agent:cursor` · `persist_zhao_prints.js --apply` |
