...
发现 → 04 记债/问题（可选）→ 03 登记 REQ/CHG/REJ
    ...
    → 实现 + 自测
    → 07 如需审阅
    → commit（带 REQ-NNN）→ 03 done + 05 Done + 自动接续下一高优项
```

### 1.1 高优队列流转与接续 SOP（长期固定机制）

为确保人机协作始终聚焦最核心价值产出，系统在 [`05-wip-board.md`](./05-wip-board.md) §0 设立**「处理中高优队列（Priority In-Flight Queue）」**：

1. **容量与排序**：队列严格维持 2~3 个任务，按 Human 与 Agent 共同评估的绝对优先级顺位排队；
2. **最高执行权**：排在第 1 顺位的任务享有当前最高开发与审查优先级；
3. **完成出队与主动接续**：
   - 当队列内任务开发与验证完成（Done）并收工出队后；
   - Agent **绝不静默停滞**，必须主动审视全局需求池（`03-requirements.md` 中的 accepted/proposed 待办项）；
   - 评估当前最紧迫、依赖已消除的高优项，拉入优先队列候选；
   - **主动向 Human 询问确认**：“【队列接续提议】上一个任务已完成出队。我建议将 `[REQ-xxx]` 调入优先队列并开始执行，您是否同意继续？”；
   - Human 确认后立即更新看板翻牌为 `Doing` 并开始执行。
