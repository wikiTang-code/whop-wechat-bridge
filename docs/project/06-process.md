...
2. **战略门禁（Strategic DoD）**：
   - 对照该任务立项时定义的实战/业务 North Star 核心成效指标（如系统性能指标、业务闭环率、实战指导置信度等）；
   - 凡主观或客观未达实战预期、关键假设未验证、或存在实质业务效果差距时，战略门禁为 **Gap Pending**；
   - 必须在报告与汇报中附带标准 `Strategic Gap Audit` 章节，由 Human 拍板是追加攻坚、接受现状（`accepted-with-gap`）、还是转入下一战略阶段。
3. **状态机细化（看板禁止只写空洞的 Done）**：
   - `done-eng`：工程与单测完成，但存在客观业务/实战 Gap，等待战略决策；
   - `done-strat`：战略指标完全达标，或 Human 明确签收接受 Gap；
...
**文档树感知（强制 · 每次同步 · `CHG-013`）**

1. `git pull` / 读盘前：**禁止**沿用聊天或会话记忆中的旧队列。  
2. 必须重读 [`05-wip-board.md`](./05-wip-board.md) **§0 全文**（0.A / 0.B / 0.H / **0.X Blocked** / 0.R）与 [`README.md`](./README.md) 队列镜像。  
3. 若本回合改了队列：同一提交内刷新 `README` 一页总览，与 §0 **逐行一致**。  
4. §0 与主看板 / 03 冲突时：**以 §0 为准先纠偏**，再改代码。  
5. 聊天结论不算数。  
6. **Blocked 可见性**：任务一旦 `Blocked`，必须在 05 **§0.X** 写清「被阻塞方 / 解锁 Owner / 解锁动作」；README 镜像点名 §0.X；禁止只在聊天里说「等对方」。  
7. **增量解锁（`AGENTS.md` §3）**：长批上游不必全量跑完；阶段产物（如部分 TSLA VL `ok`）即可触发下游；§0.X 解锁动作应写清「增量门槛」而非「等全量」。
...
### 1.3 流水线解耦与增量消费原则（`CHG-029` · Top 级流水原则）

1. **解耦阻塞**：批量长任务（真图多模态批跑、全库蒸馏、历史回放等）与下游消费任务之间**严禁完全阻塞等待**；
2. **阶段产物即流转**：上游流水产出具备一定规模的有效阶段产物（如部分关键标的落库、批次完成）时，下游即可同步开跑（图文对齐、点位提炼、归因实验或雷达校验）；
3. **状态机与游标防漏**：必须依赖严密的状态机标记（`status='ok'`、游标、主键去重），下游统一通过状态过滤增量消费；上游后台平稳推进，新产物无缝合流，不重不漏。
