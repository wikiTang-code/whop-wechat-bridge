...
| **1s OHLCV hot（CHG-056）** | **gcp-vm**（合并后 Gemini 挂采集） | **gcp-vm** `data/market/hot/`（60–90d） | jsonl + `data/manifest/market_1s.jsonl` | **未部署**；Cloud 只交脚本。采集进程不接 rclone；hot 有文件后再 copy |
