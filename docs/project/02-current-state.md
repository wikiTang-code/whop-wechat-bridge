...
| 提交 push / 生产 ff | ✅ 已 push；**REQ-002 Done**（Human 确认生产 restart） |
...
|----|-----|
| 分支 | `main` |
| HEAD / vs origin | 以 `git log -1` / `git status -sb` 为准；**CHG-056** 1s OHLCV 采集器（quote/trade 聚合，非 done-strat）；CHG-055 规划合同 + REQ-059 轨2 双账本脚手架（`done-eng`，**done-strat 未过**）；REQ-058 E-docs closed；CHG-054 ingest 落库观测 `t_arrive`（不回填） |
