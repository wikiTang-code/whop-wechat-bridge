...
| **Auto News / Persona 调度** | 强绑定于每次轮询回调内同步检测 | **当前未迁移**：`ingest_runner` 仅调用 `syncAndAnalyze`，**不会**跑 `checkAndAutoGenerateDailyNews` / `checkAndAutoUpdatePersonaPlaybook` | ⚠️ **功能缺口**：上线前必须补进 tick 或另开 cron；不可宣传为“已解耦完成” |
