...
def main():
    ...
    parser.add_argument("--code", default="US.QQQ", help="标的代码 (默认: US.QQQ)")
    ...
    try:
        ...
        print("请确认 FutuOpenD 网关客户端已经启动并成功登录富途/Moomoo 账户。")
        sys.exit(1)

    try:
        # 1) 日线: 2023-01-01 到今天, 文件 qqq_daily.csv
        print(f"\n==================== 1. 拉取日线 (Daily) ====================")
        df_daily = fetch_all_kline(quote_ctx, args.code, args.start, end_date, KLType.K_DAY, AuType.QFQ)
        daily_file = "qqq_daily.csv"
