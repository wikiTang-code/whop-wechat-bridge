...
console.log('========================================================================================');
console.log('📖 启动 8001~9000 消息段时序动态账本流水线：5-20整数底部要素 + 5-21财报电话会反转');
console.log('========================================================================================\n');

// 1. 读取 8001 ~ 9000 条赵哥时序消息 (LIMIT 1000 OFFSET 8000)
...
console.log(`👤 载入 8001~9000 条消息 (起始: ${new Date(zhaoMessages[0].created_at).toISOString().slice(0, 10)} -> 结束: ${new Date(zhaoMessages[zhaoMessages.length - 1].created_at).toISOString().slice(0, 10)})\n`);
...
for (let i = 0; i < zhaoMessages.length; i++) {
  ...
  // 2. prop_011: NVDA 电话会投资板块带动多空 (2026-05-21 post_1CbFr1hvfSzMc63eiEe5QS)
  ...
  // 4. gold_006: 节前被动减
  if (text.includes('被动减') || text.includes('节前前5天') || (text.includes('节前') && (text.includes('留现金') || text.includes('双底')))) {
...
const resultData = {
  metadata: {
    segment: '8001~9000 消息段 (2026-05-20 ~ 2026-06-18)',
