...
console.log('========================================================================================');
console.log('📚 构建全量知识资产索引与代码本汇总目录 (Zhao Codebook Summary v1.1 定版)');
console.log('========================================================================================\n');
...
// 针对 prop_011 明确移出已捕获（已降为板块映射备忘）
if (codebookSummary['prop_011_earnings_call_reversal']) {
  codebookSummary['prop_011_earnings_call_reversal'].total_replays_count = 0;
}
...
for (const [nodeId, info] of Object.entries(codebookSummary)) {
  if (info.total_replays_count > 0 && info.official_id !== 'unverified_id') {
...
const assetIndexResult = {
  metadata: {
    dataset_name: '赵哥全频道发言知识与审计全量索引表 (v1.1 定版)',
    version: '1.1',
  ...
  codebook_nodes_summary: {
    ...
    unpopulated_empty_nodes_list: unpopulatedEmptyNodes.map(u => ({
      ...
      name: u.name,
      status: u.status,
      note: u.gold_id === 'prop_006_stop_loss_sweep_method' 
        ? '暂无 8/26 四手牌总纲，仅有 6-22 散户止损单被一笔全吃实操买入点备忘' 
        : (u.gold_id === 'prop_011_earnings_call_reversal' ? '电话会投资名单已降为板块映射备忘，暂无先多后空独立机制原句' : '截至 2026-06-26 历史时序切片中尚未出现独立机制原句，实事求是保持空壳待补')
    }))
  },
...
// 生成 Markdown 汇总目录 v1.1
let mdContent = `# 🏛️ 赵哥交易受控代码本与全量知识资产汇总目录 (v1.1 定版)\n\n`;
...
mdContent += `> **执行纪律状态**：策略手册、Registry、交易执行层 L2a Blotter **保持严格冻结**；7~8 月增量另开 \`incr\` 知识档。  \n\n`;
...
mdContent += `| **实战捕获真本尊节点** | **${populatedNodes.length} 个** | 拥有独立官方 Message ID 与真实实战复述的硬核节点 |\n`;
mdContent += `| **严格保留空壳待补节点** | **${unpopulatedEmptyNodes.length} 个** | 截至 6-26 切片尚未出现独立机制原句，实事求是保持空壳不占坑 |\n`;
mdContent += `| **全量 Skip 审计记录** | **${allSkippedMemos.length} 条** | 包含全部跨频道去重、同日后缀、点位成交口播与备忘（全量资产检索原料） |\n\n`;

mdContent += `## 🌳 二、已捕获真本尊与实战证据的受控树节点明细表 (v1.1 精准对账)\n\n`;
mdContent += `| 节点代码 (\`tree_id\`) | 机制名称 | 所属章节 | 官方本尊 Message ID | 首次美东日期 | 账本实战复述/细则数 |\n`;
mdContent += `|:---|:---|:---|:---|:---:|:---:|\n`;
...
mdContent += `\n## 🈳 三、严格实事求是保持空壳待补的节点清单 (截至 2026-06-26 尚未见原句)\n\n`;
mdContent += `| 空壳节点代码 (\`tree_id\`) | 机制名称 | 状态 | 保持空壳实事求是说明 |\n`;
mdContent += `|:---|:---|:---:|:---|\n`;

unpopulatedEmptyNodes.forEach(u => {
  const reason = u.gold_id === 'prop_006_stop_loss_sweep_method'
    ? '暂无 8/26 四手牌总纲，仅有 6-22 散户止损单被吃买入点实操备忘'
    : (u.gold_id === 'prop_011_earnings_call_reversal' ? 'NVDA电话会投资名单已降为板块映射备忘，暂无先多后空独立机制原句' : '截至 2026-06-26 历史时序切片中尚未出现独立机制原句，严格不占坑、等 7~8 月增量进库后挂接');
  mdContent += `| **\`${u.gold_id}\`** | **${u.name}** | \`${u.status}\` | ${reason} |\n`;
});

mdContent += `\n## 📁 四、全量 Skip 审计资产分类分布 (全量标签资产检索底座)\n\n`;
...
Object.entries(categoryStats).forEach(([cat, count]) => {
  ...
  if (cat === 'single_ticker_memo' || cat === 'single_ticker_event_memo') desc = '单票特定点位备忘（不进宏观通用规则）';
  if (cat === 'sector_mapping_memo') desc = '电话会/个股板块映射备忘（不占多空机制节点）';
  if (cat === 'scope_refinement_memo') desc = '战法适用范围限定备忘（如二次握手针对日内低点）';
  mdContent += `| **\`${cat}\`** | **${count} 条** | ${desc} |\n`;
});

const outMdPath = 'data/l2b/gold/ZHAO_CODEBOOK_SUMMARY_V1_1.md';
fs.writeFileSync(outMdPath, mdContent, 'utf-8');
console.log(`✅ 成功输出代码本目录 Markdown v1.1: ${outMdPath}\n`);
