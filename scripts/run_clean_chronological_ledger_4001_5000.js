...
console.log('========================================================================================');
console.log('📖 启动第 6 档 (4001~5000 条) 时序动态账本流水线：受控树代码本严格对齐 + 全量 Skip 审计');
console.log('========================================================================================\n');

// 1. 读取第 4001 ~ 5000 条赵哥时序消息 (LIMIT 1000 OFFSET 4000)
...
console.log(`👤 载入 4001~5000 条消息 (起始: ${new Date(zhaoMessages[0].created_at).toISOString().slice(0, 10)} -> 结束: ${new Date(zhaoMessages[zhaoMessages.length - 1].created_at).toISOString().slice(0, 10)})\n`);
...
const BOOKMARK_REGEX = /(法\b|机制|要素|口诀|打油诗|普跌同沉|普涨我跌|事件来临|节日前夕|币市波动|一般要|一般有|相当于|二次握手|握手|缺口|只做一次|被动减|减持|总仓位不要超过|7成|3成|反弹一半|\/2=|大单检测|大单入场|散户止损|死拿|成本出|磨损值|两段式|靴子|结算|利润垫|首阴|首阳|MSCI|低吸三种|0DTE|1.5倍|腰斩|翻倍)/;
...
for (let i = 0; i < zhaoMessages.length; i++) {
  ...
  // 1. gold_004: 7/3 仓位风控 (同一天只计 1 次规则复述)
  ...
  // 4. gold_006: 节前被动减
  if (text.includes('节前被动减') || (text.includes('被动减') && (text.includes('留现金') || text.includes('双底')))) {
...
console.log(`========================================================================================`);
console.log(`✅ 成功输出第 6 档 (4001~5000 条) 树键对齐时序总账: ${outPath}`);
