...
## 6. 磁盘实存真图（>15KB）精选 5 条 L2b proposed 知识看板 (人工验图实测交底)

已人工逐一打开这 5 张物理磁盘真实存在的 jpg 图片，严格基于画面真实图元与文字据实填写 `chart_notes.markers` 和 `timeframe`：

| 样本 ID | kid | 来源消息 ID / 日期 | 磁盘真图路径 (体积 / SHA 前缀) | 赵哥发言全文 (`raw_text`) | 精确证据子串 (`evidence_span`) | 规范陈述 (`statement`) | 人工验图图表标注 (`chart_notes`) | 图文匹配判定 (`aligns`) |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| `g_img_001` | `k_nine_turn_sequence` | `post_1CXYCpXPkLs5VVnU5aBkJe`<br>(2026-01-27) | `data/media/zhao/2026-01-27/post_1CXYCpXPkLs5VVnU5aBkJe_0.jpg`<br>(**162.2 KB** / `bc6fa587...`) | `币市场 富途可以注意 等反弹的1出来时候反弹的1-9的九转序列是中线 然后8万6 8万7可以设置两个预警 最近都是 这两个短线反弹 九转序列是默认的一个数学公式 计算的曲率 反弹红1-9 回调就是绿1-9 上次大减持就是绿1出加密` | `九转序列是默认的一个数学公式 计算的曲率 反弹红1-9 回调就是绿1-9 上次大减持就是绿1出加密` | 九转序列用于计算曲率，反弹看红1-9，回调看绿1-9；出现绿1时减持防守。 | **富途BTC日K线图**，K线走势上标有九转序列上升数字(红/粉1-9)与回调数字(绿1-9) | **`match`** |
| `g_img_002` | `k_focus_on_inflection_turn` | `post_1CYDwHo9hVfbwciyfsR9sa`<br>(2026-02-17) | `data/media/zhao/2026-02-17/post_1CYDwHo9hVfbwciyfsR9sa_0.jpg`<br>(**48.9 KB** / `a50936a6...`) | `每天只需要看转弯 真金白银是真 消息都是阻碍你` | `每天只需要看转弯 真金白银是真 消息都是阻碍你` | 每天只需要看转弯，真金白银是真，消息都是阻碍你。 | **SPY分时走势图**，低位两个红色箭头标注向上转弯拐点，下方附文字“两次转弯都是有效的就是看spy675收盘是不是收上面” | **`match`** |
| `g_img_003` | `k_second_handshake` | `post_1CayBBJeexEDaiEveHEmGa`<br>(2026-05-12) | `data/media/zhao/2026-05-12/post_1CayBBJeexEDaiEveHEmGa_0.jpg`<br>(**104.0 KB** / `908ab069...`) | `二次握手比较精确的指数spx图` | `二次握手比较精确的指数spx图` | 看二次握手用 SPX 图更精确。 | **.SPX分时图**，标注“二次握手”、“第一次7339 第二次跌还是7339 平行没跌破 一般第二次探底不破是比较确定的短线买点”，两个红色箭头指向回踩低点 | **`match`** |
| `g_img_004` | `k_event_cycle_extremes` | `post_1Cb4TAuGNsh8zYEUCgnce7`<br>(2026-05-15) | `data/media/zhao/2026-05-15/post_1Cb4TAuGNsh8zYEUCgnce7_0.jpg`<br>(**29.9 KB** / `780d2858...`) | `加密的高点 总在第一轮投票投一半时候 加密的低点一半在下轮投票日期模糊和精确的公布附近` | `加密的高点 总在第一轮投票投一半时候 加密的低点一半在下轮投票日期模糊和精确的公布附近` | 加密的高点常在第一轮投票过半时，低点常在下轮投票日期模糊和精确公布附近。 | **推文手绘周期轮廓折线图**，标注“法案失败会跌下”、“大概日期确认出来公告会反弹”、“投票当天过程中第一轮通过概率高会涨” | **`match`** |
| `g_img_005` | `k_breakout_three_elements` | `post_1CbTUayc44sNzPweAjd3QW`<br>(2026-05-27) | `data/media/zhao/2026-05-27/post_1CbTUayc44sNzPweAjd3QW_0.jpg`<br>(**130.3 KB** / `1b7da4be...`) | `最有爆发的三要素要时刻盯紧 抓机会` | `最有爆发的三要素要时刻盯紧 抓机会` | 最有爆发的三要素要时刻盯紧抓机会。 | **IREN日K走势图**，赵哥红字手写标注“iren财报70跌46 三个缺口补掉了 最低点46是整数 就要中线关注了”，4个红色箭头指向缺口与46低点 | **`partial`**<br>(图文同串但配图侧重缺口与整数点位) |

> [!NOTE]
> 1. 以上 5 条全部严格基于物理磁盘上**真实存在的大图文件（29.9KB ~ 162.2KB，全部唯一 SHA256）**，经人工逐一打开核验；
> 2. `statement` 仅提取原文核心词汇，坚决剔除任何原文未提及的指标、阈值或修饰；
> 3. `status: "proposed"`，零写入 registry，`do_not_use_as_order: true` 强制生效。
