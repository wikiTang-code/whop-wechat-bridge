...
## 6. 磁盘实存真图（>15KB）精选 5 条 L2b proposed 知识看板 (终审图审已闭环)

五张物理磁盘真图人工核验与用户终审已全部通过，明确区分了图元观察与自动下单禁词：

| 样本 ID | kid | 来源消息 ID / 日期 | 磁盘真图路径 (体积 / SHA 前缀) | 赵哥发言全文 (`raw_text`) | 精确证据子串 (`evidence_span`) | 规范陈述 (`statement`) | 图表标注 (`chart_notes.markers`) | 图文判定 (`aligns`) |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| `g_img_001` | `k_nine_turn_sequence` | `post_1CXYCpXPkLs5VVnU5aBkJe`<br>(2026-01-27) | `data/media/zhao/2026-01-27/post_1CXYCpXPkLs5VVnU5aBkJe_0.jpg`<br>(**162.2 KB** / `bc6fa587...`) | `币市场 富途可以注意 等反弹的1出来时候反弹的1-9的九转序列是中线 然后8万6 8万7可以设置两个预警 最近都是 这两个短线反弹 九转序列是默认的一个数学公式 计算的曲率 反弹红1-9 回调就是绿1-9 上次大减持就是绿1出加密` | `九转序列是默认的一个数学公式 计算的曲率 反弹红1-9 回调就是绿1-9 上次大减持就是绿1出加密` | 九转序列用于计算曲率，反弹看红1-9，回调看绿1-9；出现绿1时减持防守。 | **富途BTC日K线图(现价约87832)**，上行一段粉/紫1-9，下行一段绿2-9，右下角绿9旁有紫9，虚线在8.7万附近对应8万6/8万7预警 | **`match`** |
| `g_img_002` | `k_focus_on_inflection_turn` | `post_1CYDwHo9hVfbwciyfsR9sa`<br>(2026-02-17) | `data/media/zhao/2026-02-17/post_1CYDwHo9hVfbwciyfsR9sa_0.jpg`<br>(**48.9 KB** / `a50936a6...`) | `每天只需要看转弯 真金白银是真 消息都是阻碍你` | `每天只需要看转弯 真金白银是真 消息都是阻碍你` | 每天只需要看转弯，真金白银是真，消息都是阻碍你。 | **SPY分时图(现价约677.8)**，两支红箭头指两个低点拐点，蓝气泡发言“两次转弯都是有效的就是看spy675收盘是不是收上面”，675为观察位非下单点 | **`match`** |
| `g_img_003` | `k_second_handshake` | `post_1CayBBJeexEDaiEveHEmGa`<br>(2026-05-12) | `data/media/zhao/2026-05-12/post_1CayBBJeexEDaiEveHEmGa_0.jpg`<br>(**104.0 KB** / `908ab069...`) | `二次握手比较精确的指数spx图` | `二次握手比较精确的指数spx图` | 看二次握手用 SPX 图更精确。 | **.SPX分时图**，红字标注“二次握手”、“第一次7339 第二次跌还是7339”、“平行没跌破 一般第二次探底不破是比较确定的短线买点”、“cifr iren 都比较符合短线二次平行”，两支红箭头对着两次近7339的底；7339/CIFR/IREN为图元观察标注，严禁下单 | **`match`** |
| `g_img_004` | `k_event_cycle_extremes` | `post_1Cb4TAuGNsh8zYEUCgnce7`<br>(2026-05-15) | `data/media/zhao/2026-05-15/post_1Cb4TAuGNsh8zYEUCgnce7_0.jpg`<br>(**29.9 KB** / `780d2858...`) | `加密的高点 总在第一轮投票投一半时候 加密的低点一半在下轮投票日期模糊和精确的公布附近` | `加密的高点 总在第一轮投票投一半时候 加密的低点一半在下轮投票日期模糊和精确的公布附近` | 加密的高点常在第一轮投票过半时，低点常在下轮投票日期模糊和精确公布附近。 | **法案轮廓折线图(非K线)**，标注“法案失败会跌下”、“大概日期确认出来公告会反弹”、“小回调下几天 精确日期出来再反弹”、“投票当天 过程中第一轮通过概率高会涨”；下半推文为CONL/HOOD/BMNR减仓口播属L2a单据不并入战法 | **`match`** |
| `g_img_005` | `k_gap_fill_round_number_bottom` | `post_1CbTUayc44sNzPweAjd3QW`<br>(2026-05-27) | `data/media/zhao/2026-05-27/post_1CbTUayc44sNzPweAjd3QW_0.jpg`<br>(**130.3 KB** / `1b7da4be...`) | `最有爆发的三要素要时刻盯紧 抓机会` | `最有爆发的三要素要时刻盯紧 抓机会` | 财报大跌补齐缺口且最低点触及整数位时，中线关注。 | **IREN日K走势图**，赵哥红字手写标注“iren财报70跌46 三个缺口补掉了 最低点46是整数 就要中线关注了”，左下3个红色箭头指缺口，右下一个红色箭头指46低点 | **`partial`** |

> [!IMPORTANT]
> 1. 以上 5 条全部维持 **`proposed`**，**`do_not_use_as_order: true` 强制生效**；
> 2. 新 kid 严格不进入 `known_kids_registry.json`，不升级为 `gold`；003 图上“短线买点”等严禁作为自动下单指令；
> 3. 本批真图审核正式全部完成，工程全面停工待命。
