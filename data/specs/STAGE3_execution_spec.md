# ⚙️ Stage 3: 执行层技术规格规范 (V1.1 严格核准版)

> **版本标识**：`STAGE3-EXEC-SPEC-V1.1-APPROVED`  
> **数据基准**：严格基于 QQQ 2026-01～03 回测表四列执行开关与周哥 4 条已核实纪律构建。  
...
3. **持仓隔离**：
   - 每一笔成交作为独立的 `Lot` 进行追踪，记录开仓时间、开仓价格、出场时间、出场价格与已实现净利润 (PnL)。
...
2. **核心指标公式**：
   - **单笔净利润 (PnL)**：$\text{PnL} = \text{Amount} \times (P_{\text{exit}} - P_{\text{entry}}) - \text{Fee}$
   - **胜率 (Win Rate)**：$\text{Win Rate} = \frac{\text{Count}(\text{PnL} > 0)}{\text{Total Closed Lots}}$
   - **硬止损率 (Stopout Rate)**：触发硬止损被动平仓的比例（注：与到期平仓分别统计）。
...
| :--- | :--- | :--- | :--- |
| **RTH·locked** | 常规交易时段 (09:30–16:00 ET) | 严格锁定基准过滤 | 传统美股主交易时段基准线 |
...
1. **推荐扫描窗口**：
   - **夜盘与盘前窗口 (Overnight / Pre-Market)**：盘后静止数据避免了日内分时假突破的干扰，为批量扫描与体制计算的推荐窗口；（RTH·locked 为主盘交易时段对照组）；
2. **限价挂单模式 (Limit Order Policy)**：
   - **中性贴盘限价 (Neutral Limit)**：严格以买一/卖一档位挂单等待被动成交；
   - **严禁市价追单 (Market Orders Prohibited)**：严禁以市价单扫盘。
...
2. **物理隔离**：本模块产出的所有回测实验与状态，**严格独立写入 `data/runs/mrzhou_strategy/`**，严禁回写 `l2a_order_candidates` 或推入券商实盘交易链路！
