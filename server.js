...
import { generatePersonaPlaybook, getPersonaStatus, processPersonaTask, resumePersonaPlaybook } from './persona-engine.js';
...
async function checkAndAutoGenerateDailyNews() {
  try {
    ...
    // 检查今天是否已经有该类型的报告或有正在运行的任务
    const isAlreadyScheduledOrDone = (type) => {
      // 1. 查已生成成功的总结
      const summary = db.prepare(`
        SELECT id FROM news_summaries 
        WHERE summary_type = ? AND created_at >= ?
        LIMIT 1
      `).get(type, todayStartMs);
      
      if (summary) return true;
      
      // 2. 查队列中是否有相同类型的 news_reduce 任务在排队或运行
    ...
    // 1. 盘前速报 (briefing) - 北京时间周一到周五，17:30 以后
    if (['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].includes(weekday)) {
      if (hour > 17 || (hour === 17 && minute >= 30)) {
        if (!isAlreadyScheduledOrDone('briefing')) {
          console.log(`[Auto News Scheduler] Triggering automated daily briefing summary at ${hour}:${minute} HKT...`);
          await generateNewsSummary('briefing').catch(err => {
            console.error('[Auto News Scheduler] Failed to trigger briefing summary:', err.message);
          });
        }
      }
    }
    
    // 2. 盘中总结 (intraday) - 北京时间周二到周六，01:30 以后
    if (['Tue', 'Wed', 'Thu', 'Fri', 'Sat'].includes(weekday)) {
      if (hour > 1 || (hour === 1 && minute >= 30)) {
        if (!isAlreadyScheduledOrDone('intraday')) {
          console.log(`[Auto News Scheduler] Triggering automated daily intraday summary at ${hour}:${minute} HKT...`);
          await generateNewsSummary('intraday').catch(err => {
            console.error('[Auto News Scheduler] Failed to trigger intraday summary:', err.message);
          });
        }
      }
    }
    
    // 3. 收盘回顾 (closing) - 北京时间周二到周六，08:30 以后
    if (['Tue', 'Wed', 'Thu', 'Fri', 'Sat'].includes(weekday)) {
      if (hour > 8 || (hour === 8 && minute >= 30)) {
        if (!isAlreadyScheduledOrDone('closing')) {
          console.log(`[Auto News Scheduler] Triggering automated daily closing summary at ${hour}:${minute} HKT...`);
          await generateNewsSummary('closing').catch(err => {
            console.error('[Auto News Scheduler] Failed to trigger closing summary:', err.message);
          });
        }
      }
    }
...
async function runAdaptivePoll() {
  ...
  try {
    ...
    console.error('[调度器] syncAndAnalyze / checkAndAutoGenerateDailyNews 异常:', err.message);
