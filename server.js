...
async function checkAndAutoGenerateDailyNews() {
  try {
    ...
    // 1. 获取今天在上海时区的星期几 (1=Mon, 2=Tue, ..., 6=Sat, 7=Sun)
    ...
    let currentDayOfWeek = parseInt(parts.weekday, 10);
    ...
    // 计算本周所有需要覆盖的日子 (从周一到今天)
    // 如果今天是周日 (7)，我们最多补到周六 (6) 的收盘回顾
    const checkLimitDay = Math.min(currentDayOfWeek, 6); 
    
    for (let d = 1; d <= checkLimitDay; d++) {
      const diffDays = currentDayOfWeek - d;
      const targetDayMs = todayStartMs - (diffDays * 24 * 60 * 60 * 1000);
