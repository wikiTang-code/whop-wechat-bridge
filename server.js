...
// Background poller task runner (CHG-061: three independent HOT/WARM/COLD loops)
const pollerState = {
...
function channelsForLiveTier(tier) {
  try {
    loadChannelRegistry();
  } catch (err) {
...
async function runTierPoller(tier, { runHousekeeping = false } = {}) {
  ...
  const scheduleNext = () => {
    ...
    const ms = getPollIntervalMs(tier, {
      isTrading,
      throttleCold,
      coldThrottleMs: throttleCold ? getEffectivePollIntervalSec() * 1000 : null
    });
    state.timeout = setTimeout(() => runTierPoller(tier, { runHousekeeping }), ms);
  };
  ...
  try {
    ...
    if (group.length > 0) {
      ...
      const result = await syncChannelGroup(group, { limit, skipReport: true });
      if (result && result.success) {
        setLastSyncTime(Date.now());
      }
      const n = result?.newMessagesCount || 0;
      if (n > 0) {
        console.log(`[CHG-061 ${tier}] +${n} new · next ${getPollIntervalMs(tier, { isTrading: isUSTradingHours() })}ms`);
      }
    }
    if (runHousekeeping) {
...
function startPoller() {
  for (const tier of ['hot', 'warm', 'cold']) {
    if (pollerState[tier].timeout) clearTimeout(pollerState[tier].timeout);
  }
  console.log('Starting CHG-061 HOT(2s)/WARM(5s)/COLD(30s) pollers. HOT fires immediately.');
  runTierPoller('hot', { runHousekeeping: false });
  setTimeout(() => runTierPoller('warm', { runHousekeeping: false }), 2000);
  setTimeout(() => runTierPoller('cold', { runHousekeeping: true }), 4000);
}
...
function isUSTradingHours() {
  ...
  // 必须显式指定 hourCycle: 'h23'，否则 00:00~00:59 在部分环境返回 hour: '24' 导致判断失真！
  ...
  const hour = (parseInt(parts.hour, 10) || 0) % 24; // 双重防护取模 % 24

  // 周日: 完全休市
  if (weekday === 'Sun') return false;

  // 周六: 仅 00:00~08:00 活跃 (对应美股周五常规盘 9:30-16:00 ET 与盘后 16:00-20:00 ET)
